...
export SIM_DEVICE_ID=$(xcodebuild -project Single-Application-Project-AllTargets.xcodeproj -scheme Single-Application-UnitTests -showdestinations -destination "generic/platform=iOS Simulator" | grep "platform:iOS Sim" | head -1 | ruby -e "puts STDIN.read.split(',')[1].split(':').last")
