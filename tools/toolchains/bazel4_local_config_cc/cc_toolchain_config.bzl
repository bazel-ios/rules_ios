...
def _impl(ctx):
    if (ctx.attr.cpu == "ios_arm64"):
        ...
        # HACK(ios_sim_arm64) - Bazel want's to validate platforms
        # So we can't actually set the CPU in the toolchain for now. We build
        # under Bazel4 x86_64 as arm64 with these flags
        if "bazel4.override_simulator_cpu_arm64" in ctx.features:
