...
def _get_framework_files(ctx, deps):
    ...
    for dep in deps:
        ...
        for modulemap in dep[apple_common.Objc].direct_module_maps:
            if modulemap.owner == dep.label:
                ...
                # from the attr
                continue
            modulemaps_in = modulemap

    binary_out = paths.join(framework_dir, framework_name) if binaries_in else None
    modulemaps_out = [paths.join(framework_dir, "Modules", "module.modulemap")] if modulemaps_in else None
    ...
    modulemaps_out = _framework_packaging_multi(ctx, "modulemap", modulemaps_in, modulemaps_out, framework_manifest)
