...
load("//rules:providers.bzl", "AvoidDepsInfo", "FrameworkInfo", "PrivateHeadersInfo")
...
def _get_symlinked_framework_clean_action(ctx, framework_files, compilation_context_fields):
    ...
    framework_root = find_framework_dir(framework_contents)
    if framework_root:
        ctx.actions.run(
...
def _apple_framework_packaging_impl(ctx):
    ...
    framework_files = get_framework_files(ctx, ctx.attr, deps)
    outputs = framework_files.outputs
