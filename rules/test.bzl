...
def _make_test_suite_splits(factory, name, **in_kwargs):
    ...
    """
    if "runners" in in_kwargs and "runner" in in_kwargs:
    ...
    if runners and len(runners) > 0:
        ...
        for runner in runners:
            ...
            for in_test_name, in_split in splits:
...
default_test_suite_factory = struct(
...
def _ios_test(name,  bundle_rule, test_rule, test_suite_factory, apple_library, infoplists_by_build_setting = {}, split_name_to_kwargs = {}, internal_test_deps = [], **kwargs):
    """
    ...
    def _make_test(test_name, **kwargs):
    ...
    if "runners" in ios_test_kwargs or "split_name_to_kwargs" in ios_test_kwargs:
        test_suite_factory.make_test_suite(test_suite_factory, name, _make_test, **ios_test_kwargs)
    else:
        _make_test(name, **ios_test_kwargs)

def ios_unit_test(name, apple_library = apple_library, test_suite_factory = default_test_suite_factory, **kwargs):
    """
    ...
    """
    _ios_test(name, _ios_internal_unit_test_bundle, _ios_unit_test, test_suite_factory, apple_library, **kwargs)

def ios_ui_test(name, apple_library = apple_library, test_suite_factory = default_test_suite_factory, **kwargs):
    """
    ...
    _ios_test(name, _ios_internal_ui_test_bundle, _ios_ui_test, test_suite_factory, apple_library, **kwargs)

def ios_unit_snapshot_test(name, apple_library = apple_library, test_suite_factory = default_test_suite_factory, **kwargs):
    """
    ...
    Args:
        ...
        **kwargs: Arguments passed to the apple_library and ios_unit_test rules as appropriate.
    """
    _ios_test(name, _ios_internal_unit_test_bundle, _ios_unit_test, test_suite_factory, apple_library, internal_test_deps = ["@bazel_tools//tools/cpp/runfiles"], **kwargs)
