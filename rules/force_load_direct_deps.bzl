...
def _cc_info_static_libraries(dep):
    ...
    These include only .a library files that are direct deps of the target, and NOT imported static libraries.
    CcInfo only provides `.static_library` and does not make this distinction, we infer it here based on the path.
    """
