...
def pack_tree_artifact(ios_application_output_path, app_name):
    ...
    archive_root = os.environ.get(
        ...
        os.path.join(
            os.path.dirname(ios_application_output_path), "lldb-test-intermediate"
        ),
    )
...
def run_app_in_simulator(
    ctx, simctl_path, simulator_udid, ios_application_output_path, app_name
):
    ...
    with sim_template.extracted_app(
        pack_tree_artifact(ios_application_output_path, app_name), app_name
    ) as app_path:
        logger.debug("Installing app %s to simulator %s", app_path, simulator_udid)
        subprocess.run([simctl_path, "install", simulator_udid, app_path], check=True)
        app_bundle_id = sim_template.bundle_id(app_path)
        logger.info("Launching app %s in simulator %s", app_bundle_id, simulator_udid)
        args = [
            simctl_path,
            "launch",
            "--wait-for-debugger",
        ...
        simctl_process = subprocess.Popen(
            args,
            env=sim_template.simctl_launch_environ(),
            stdout=subprocess.PIPE,
...
def sim_thread_main(
    ctx, sim_device, sim_os_version, ios_application_output_path, app_name, minimum_os
):
    with temporary_ios_simulator(
        ctx, ctx.simctl_path, sim_device, sim_os_version
    ) as simulator_udid:
        run_app_in_simulator(
            ctx, ctx.simctl_path, simulator_udid, ios_application_output_path, app_name
        )
...
class TestContext:
    def __init__(self, pid, app_name, test_root):
        ...
        if "SDKROOT" in os.environ:
            self.developer_dir = os.path.dirname(
                os.path.dirname(
                    os.path.dirname(
                        os.path.dirname(os.path.dirname(os.environ["SDKROOT"]))
                    )
                )
            )
        else:
        ...
        self.simctl_path = os.path.join(self.developer_dir, "usr", "bin", "simctl")
...
def attach_debugger(ctx, test_root, pid, lldbinit, stdout):
    ...
    logger.info(
        "spawning LLDB with args: cd %s && %s - init %s",
        test_root,
        lldb_desc + " - " + src_str,
        lldbinit,
    )
    ...
    lldb_process.stdin.write(("command source " + str(lldbinit) + "\n").encode("utf-8"))
...
def cleanup(ctx):
    ...
    subprocess.run(
        [ctx.simctl_path, "shutdown", udid], stderr=subprocess.DEVNULL, check=False
    )
...
def run_lldb(app_path, sdk, device, lldbinit_path, test_root, lldb_stdout):
    ...
    try:
        ...
        attach_debugger(
            ctx,
            test_root=ctx.GetTestRoot(),
            pid=ctx.GetStartedAppPid(),
            lldbinit=lldbinit_path,
            stdout=lldb_stdout,
        )
