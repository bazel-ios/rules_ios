...
def setup_test_tmp_dir(exec_root, test_tmp_dir, test_spec):
    ...
    br_path = os.path.join(test_tmp_dir, "breakpoint.py")
    logging.info("Symlinking breakpoint %s to %s", breakpoint_file, br_path)
    os.symlink(breakpoint_file, br_path)
    # Big hack here - this should use an abs path - thats not trival ATM
    try:
        os.remove(os.path.join(exec_root, "breakpoint.py"))
        #os.symlink(breakpoint_file, os.path.join(exec_root, "breakpoint.py"))
    except:
        pass
