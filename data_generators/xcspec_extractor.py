...
def xcspec_from_file(path):
    xml = subprocess.check_output(["plutil", "-convert", "xml1", "-o", "-", "--", path])
...
def generate_method_body_for_expression(string, key):
    ...
    if {key} in xcconfigs:
        ...
        if "DefaultValue" in opt:
            ...
            used_user_content = used_user_content or {var}_used_user_content""".format(
                var=var, name=name, key=key, key_value=format_str(name)
            )
...
def add_eval(id, name, key, string, eval_methods):
    method_name = "__".join((id.replace(".", "_"), name.replace(".", "_"), key))
...
""".format(
        ...
        string=string,
        expr=generate_method_body_for_expression(string, key),
    )
        ...
        k: (
            {
                o["Name"]: {
                    ok: (
                        add_eval(x["Identifier"], o["Name"], ok, ov, eval_methods)
                        if ok in ("DefaultValue", "Condition")
                        else ov
                    )
